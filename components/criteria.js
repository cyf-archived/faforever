    ...
    return <div>
      <Spin spinning={this.props.music.loading}>
        <ul>
          ...
          <Divider />
          <li key={'__cached__'} className={this.props.music.current_criteria === '__cached__' ? 'active' : ''} onClick={
            () => {
              this.props.music.toggle('__cached__');
            }} >
            <Icon type="cloud-download" />
            <span>已缓存</span>
            <Tag>{this.props.music.songs['__cached__'] ? this.props.music.songs['__cached__'].length : 0}</Tag>
          </li>
        </ul>

    ...
    return (
      <div className={sty.container}>
        ...
        <List
          ...
          renderItem={item => (<List.Item key={`${item.id}-${item.cached ? '1' : '2'}`} onDoubleClick={() => {
            ...
            <div className='duration'>
              {`${Math.floor(Number(item.additional.song_audio.duration) / 60)}:${item.additional.song_audio.duration % 60 < 10 ? '0' + item.additional.song_audio.duration % 60 : item.additional.song_audio.duration % 60}`}
              { item.cached && <Icon type="cloud-download" style={{ marginLeft: 4, color: '#c62f2f', fontSize: 12 }} /> }
            </div>
          </List.Item>)}
        >
        </List>
